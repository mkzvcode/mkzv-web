...
export default function AboutPage() {
  return <><Header active="Обо мне" /><main className="page-shell page-grid border-x border-[var(--line)] px-4 py-8 md:px-6 md:py-10">
    <section className="about-head grid gap-8 border-b border-[var(--line)] pb-10 lg:grid-cols-[minmax(0,0.74fr)_minmax(320px,0.48fr)]"><div><p className="mono mb-7 text-[var(--muted)]">04. About / why MKZV</p><h1 className="display case-title">Сайт должен<br />продавать</h1></div><p className="copy self-end text-[var(--muted)]">Я Максим. Собираю сайт как систему: смысл, дизайн, код, скорость, адаптив и путь до заявки.</p></section>
    ...
    <section className="section about-approach grid gap-7 border-t border-[var(--line)] pt-10 lg:grid-cols-[150px_minmax(0,1fr)]"><p className="mono text-[var(--paper)]">02. Подход</p><p className="copy-lg max-w-5xl text-[var(--muted)]">Красивый сайт не спасает плохой оффер. Сначала смысл. Потом сетка, экран, код и запуск.</p></section>
    <section className="grid gap-3 pb-10 sm:grid-cols-2 xl:grid-cols-4">{clientSignals.map((item) => <div className="metric-tile panel p-5" key={item.value}><p className="display text-[clamp(2rem,3vw,3.5rem)]">{item.value}</p><p className="mono mt-6 leading-5 text-[var(--muted)]">{item.label}</p></div>)}</section>
